...
#include "archive/extractcallback.h"
#include <Unknwn.h>

#include "archive/archive.h"
#include "archive/propertyvariant.h"
...
CArchiveExtractCallback::CArchiveExtractCallback(ProgressCallback* /*progressCallback*/,
    ...
    : m_DirectoryPath(directoryPath), m_Extracting(false), m_Canceled(false),
      ...
      m_FileChangeCallback(fileChangeCallback), m_ErrorCallback(errorCallback), m_PasswordCallback(passwordCallback) {}
...
template <typename T>
bool CArchiveExtractCallback::getOptionalProperty(UInt32 index, int property, T* result) const {
...
template <typename T>
T CArchiveExtractCallback::getProperty(UInt32 index, int property) const {
    PropertyVariant prop;
    if (m_ArchiveHandler->GetProperty(index, property, &prop) != S_OK) {
...
//        } break;
//        default: { reportError("Unknown error"); } break;
//        }
//    }
//
...
void CArchiveExtractCallback::SetCanceled(bool aCanceled) { m_Canceled = aCanceled; }
